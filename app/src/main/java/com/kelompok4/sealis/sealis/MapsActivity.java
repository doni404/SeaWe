        ...
        if (mMap == null) {
            ...
            if (mMap != null) {
                ...
                task.execute (new String[ ]
                                {"http://buangdisini.com/papb/webservice.php?idstasiun="+stasiunID}
                );
            }
        }
    }
    ...
    public void setUpMap(double lat,double lon) {
        if(!wilayah.isEmpty()) {
            ...
            for (Wilayah wil : wilayah) {
                Marker marker = mMap.addMarker(new MarkerOptions().position(new LatLng(wil.lat, wil.lon)).title(wil.nama).snippet(wil.stasiun));
        ...
        protected void onPostExecute(String result) {
            ...
            if (!response.equals("null")) {
                try {
                    ...
                    JSONArray jArr = jObj.getJSONArray("area");
                    if (jArr.length()!=0) {
                        for (int i = 0; i < jArr.length(); i++) {
                            JSONObject area = jArr.getJSONObject(i);
                            int id = area.getInt("id");
                            String stasiun = area.getString("stasiun");
                            String nama = area.getString("wilayah");
                            lat = Double.parseDouble(area.getString("lat"));
                            lon = Double.parseDouble(area.getString("long"));
                        ...
                        String kecangin = info.getString("kecangin");
                        String tinggigel = info.getString("tinggigel");
                        String pcuaca = prediksi.getString("cuaca");
                        String parahangin = prediksi.getString("arahangin");
                        String pkecangin = prediksi.getString("kecangin");
                        String ptinggigel = prediksi.getString("tinggigel");
